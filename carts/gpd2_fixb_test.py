...
def main():
    ...
    cfg = MoEConfig(skew=1.5, seed=0, num_gpus=16, gpus_per_domain=2, num_experts=16)
    dem = generate_demand(cfg)

    p_c0 = greedy_crp(dem, CRPConfig(relay_capacity=0))
    c0 = run_variant("gpd2fb_c0", p_c0, WriterConfig(dpu_bytes_per_us=100000))

    print("=" * 86)
    print("gpus_per_domain=2 (8 domains), cap=120: OLD link-only vs NEW cost-aware greedy")
    print("=" * 86)
    print(f"C0 baseline: {c0} cycles\n")
    print(f"{'dpu_bytes_per_us':>16} | {'OLD cycles':>10} | {'OLD vs C0':>10} | {'NEW cycles':>10} | {'NEW vs C0':>10} | winner(OLDvsNEW)")
    print("-" * 100)
    for dbpu in DPU_SWEEP:
        ...
        c_old = run_variant(f"gpd2fb_old_{dbpu}", p_old, wcfg)
        c_new = run_variant(f"gpd2fb_new_{dbpu}", p_new, wcfg)
        if c_old is None or c_new is None:
            print(f"{dbpu:>16,} | FAILED")
            continue
        old_vs_c0 = 100.0 * (c0 - c_old) / c0
        new_vs_c0 = 100.0 * (c0 - c_new) / c0
        delta = 100.0 * (c_old - c_new) / c_old
        winner = "NEW" if c_new < c_old else ("OLD" if c_new > c_old else "TIE")
        print(f"{dbpu:>16,} | {c_old:>10} | {old_vs_c0:>+9.2f}% | {c_new:>10} | {new_vs_c0:>+9.2f}% | "
              f"{winner} (delta {delta:+.2f}%)")
