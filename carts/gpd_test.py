...
def main():
    ...
    p_c3 = greedy_crp(dem, CRPConfig(relay_capacity=CAP))

    emit_ets(os.path.join(SYSDIR, "gpd2_c0"), "gpd2_c0", p_c0, wcfg)
    write_system_json("gpd2_c0")
    emit_ets(os.path.join(SYSDIR, "gpd2_c3"), "gpd2_c3", p_c3, wcfg)
    write_system_json("gpd2_c3")

    c0 = run_cycles("gpd2_c0")
    c3 = run_cycles("gpd2_c3")
    if c0 is None or c3 is None:
        print("FAILED")
        return
    gain = 100.0 * (c0 - c3) / c0
    print("=" * 60)
    print(f"gpus_per_domain=2 (8 domains), cap={CAP}, dpu_bytes_per_us={DPU}")
    print("=" * 60)
    print(f"  C0 baseline : {c0:>9} cycles")
    print(f"  C3 CARTS    : {c3:>9} cycles")
    print(f"  gain%       : {gain:>+8.2f}%")
    print(f"  (for comparison: load-level gain was 6.36%, from cap-sweep)")
