...
OUT  = "fig_necessity_composite.pdf"
BWS  = [5, 10, 20, 50]
...
def heat(ax, d, title):
    M = [[cell(d, bw, dpu)[0] for dpu in DPUS] for bw in BWS]
    G = [[cell(d, bw, dpu)[1] for dpu in DPUS] for bw in BWS]
    im = ax.imshow(M, cmap="RdBu", norm=norm, aspect="auto")
    ax.grid(False)
    ax.set_xticks(range(3)); ax.set_xticklabels(DPU_LBL)
    ax.set_yticks(range(4)); ax.set_yticklabels(["%d" % b for b in BWS])
    ...
    for i in range(4):
        for j in range(3):
            ...
            ax.text(j, i - 0.12, "%+.1f" % v, ha="center", va="center",
                    fontsize=8, color=col)
            ax.text(j, i + 0.26, "%d/6" % g, ha="center", va="center",
...
cb.set_label("mean gain vs C0 (%)", fontsize=8)
fig.savefig(OUT)
print("wrote", OUT)
